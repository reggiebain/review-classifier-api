...
def get_logs():
    try:
        ...
        c = conn.cursor()
        c.execute("SELECT timestamp, review, prediction FROM predictions ORDER BY timestamp DESC LIMIT 100")
        rows = c.fetchall()
        conn.close()
        return [
            {"timestamp": r[0], "review": r[1], "prediction": r[2]} for r in rows
        ]
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def log_prediction(text: str, prediction: int):
    ...
    c.execute("INSERT INTO predictions (timestamp, review, prediction) VALUES (?, ?, ?)",
